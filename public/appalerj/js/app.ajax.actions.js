...
app.ajax.agendaInternaSuccess = function (result, textStatus, jqXHR) {
    ...
    $('.controle-conteudo')
        .find('.data')
        .append(result.DataInicio + ' - ' + result.DataFim)

    $('.container-fluid.conteudo').html('')
    $('.container-fluid.conteudo').append(
        '<div align="center"><p>' + result.Descricao + '</p></div>',
    )
...
app.ajax.agendaPesquisaSuccess = function (result, textStatus, jqXHR) {
    ...
    var templateResultado =
        ' <div class="resultado"><h2><a href="agenda_interna.html" class="agenda-resultado" data-transition="slide" data-alerj-id="$$_ALERJ_ID_$$" >$$_TITULO_$$</a></h2>' +
        '<p>$$_DESCRICAO_$$</p></div>'

...
function montaRegimeInternoPesquisa(regime) {
    ...
    var templateCapitulo =
        '<a $$_LINK_$$ data-transition="slide" class="capitulo ordem-item $$_SEM_LINK_$$" data-titulo="$$_TITULO_$$" data-alerj-id="$$_ID_$$">' +
        '<span class="area">$$_CAPITULO_$$</span>' +
        '<span class="descricao">$$_DESCRICAO_CAPITULO_$$</span>' +
        '</a>'

    var templateSecao =
        '<a $$_LINK_$$ data-transition="slide"  class="secao ordem-item $$_SEM_LINK_$$" data-titulo="$$_TITULO_$$" data-alerj-id="$$_ID_$$">' +
        '<span class="area">$$_SECAO_$$</span>' +
        '<span class="descricao">$$_TITULO_SECAO_$$</span>' +
        '</a>'

    var templateSubSecao =
        '<a $$_LINK_$$ data-transition="slide"  class="subsecao ordem-item $$_SEM_LINK_$$" data-titulo="$$_TITULO_$$" data-alerj-id="$$_ID_$$">' +
        '<span class="area">$$_SECAO_$$</span>' +
        '<span class="descricao">$$_TITULO_SECAO_$$</span>' +
        '</a>'
...
function montaRegimeInterno() {
    ...
    var templateCapitulo =
        '<a $$_LINK_$$  class="capitulo ordem-item $$_SEM_LINK_$$" data-titulo="$$_TITULO_$$" data-alerj-id="$$_ID_$$">' +
        '<span class="$$_AREA_$$">$$_CAPITULO_$$</span>' +
        '<span class="descricao">$$_DESCRICAO_CAPITULO_$$</span>' +
        '</a>'

    var templateSecao =
        '<a $$_LINK_$$ class="secao ordem-item $$_SEM_LINK_$$" data-titulo="$$_TITULO_$$" data-alerj-id="$$_ID_$$">' +
        '<span class="area">$$_SECAO_$$</span>' +
        '<span class="descricao">$$_TITULO_SECAO_$$</span>' +
        '</a>'

    var templateSubSecao =
        '<a $$_LINK_$$ class="subsecao ordem-item $$_SEM_LINK_$$" data-titulo="$$_TITULO_$$" data-alerj-id="$$_ID_$$">' +
        '<span class="area">$$_SECAO_$$</span>' +
        '<span class="descricao">$$_TITULO_SECAO_$$</span>' +
        '</a>'
...
function ajustaTamanhoNoticia() {
    ...
    $.each($elemPrimeiro, function (index, elemVal) {
        ...
        $str =
            ...
            '</span>' // Recompose the string with the span tag wrapped around the hidden part of it
        $elem.html($str)
    })
    ...
    $('.botao-expandir').click(function () {
        ...
        $(this)
            .parent()
            .find('.animar-conteudo p.limit span.texto')
...
function montaListaNoticias() {
    ...
    $.each(app.ajax.listaNoticias, function (index, noticia) {
        if (index <= 2) {
            ...
            if (
                noticia.Multimidias &&
                noticia.Multimidias[0] &&
                noticia.Multimidias[0].Arquivo != null &&
                noticia.Multimidias[0].Credito != null
            ) {
                templateNoticia = templateNoticia.replace(
                    '$$_DESC_IMG_$$',
                    noticia.Multimidias[0].Credito,
                )
                templateNoticia = templateNoticia.replace(
                    '$$_POR_$$',
                    noticia.Multimidias[0].Credito,
            ...
            $.each(noticia.Multimidias, function (index, multimidia) {
                var img = $("<img class='imagem img-responsive' />")
                    .attr('src', 'http://' + multimidia.Arquivo.trim())
                    .on('load', function () {
                        if (
                            !this.complete ||
                    ...
                    .on('error', function () {
                        ...
                        $('#' + noticia.ID).html('')
                        $('#' + noticia.ID).append(
                            '<img class="img-responsive" src="imagens/img_indisponivel.jpg" >',
                        )
                    })
...
function montaDeputadoInterna(deputado) {
    ...
    var img = $("<img class='img-responsive' />")
        .attr('src', 'http://' + deputado.Foto.trim())
        .on('load', function () {
            if (
                !this.complete ||
                typeof this.naturalWidth == 'undefined' ||
        ...
        .on('error', function () {
            $('#deputado-interna-imagem').remove()
            $('#controle-deputado-interna-imagem').append(
                '<img class="img-responsive" src="imagens/img_indisponivel.jpg" >',
            )
        })
    ...
    if (deputado.Email && deputado.TelefoneInstitucional[0]) {
        ...
        $('.telefone').html(
            '<a href="tel:+55-' +
                deputado.TelefoneInstitucional[0]
                    .replace('(', '')
                    .replace(')', '')
                    .replace(' ', '-') +
                '">' +
    ...
    if (deputado.TelefoneInstitucional[0]) {
        endereco_rodape =
            endereco_rodape +
            'Telefone: <a href="tel:+55-' +
            deputado.TelefoneInstitucional[0].replace('(', '').replace(')', '').replace(' ', '-') +
            '">' +
            deputado.TelefoneInstitucional[0] +
            '</a></br>'
    }
    ...
    $.each(app.ajax.listaOrdens, function (index, value) {
        if (value.carbon && value.carbon.date) {
            if (dataOrdemSelecionada == formatDate(new Date(value.carbon.date.replace(' ', 'T')))) {
                ...
                templateOrdem =
                    '<li><a class="ordem-item" data-alerj-id="$$_ALERJ_ID_$$">' +

...
app.actions.ordemPesquisa = function (filtro) {
    ...
    var templateResultado =
        ' <div class="resultado"><h2><a class="agenda-resultado" data-alerj-id="$$_ALERJ_ID_$$" >$$_TITULO_$$</a></h2>' +
        '<p>$$_DESCRICAO_$$</p></div>'
    ...
    $.each(resultado, function (i, obj) {
        if (i <= 30) {
            ...
            var descricao = obj.document.substring(
                obj.document.toUpperCase().indexOf(filtro.toUpperCase()) - 30,
                obj.document.toUpperCase().indexOf(filtro.toUpperCase()) + 30,
            )
...
app.actions.ajax = function (config) {
    ...
    if (app.crossdomain) {
        _dataType = 'jsonp'
        _jsonpCallback =
            "app.actions.callbackJsonp('" + config.jsonpCallback + "', true, '$JSON_param')"
...
app.actions.navigateIndex = function (type, message) {
    $.mobile.changePage(app.media.util.indexMediaPage(), {
        transition: 'slide',
        reverse: false,
        changeHash: true,
    })

    ...
    $.position = {
        ...
        getScrollInfo: function (within) {
            var overflowX =
                ...
                overflowY =
                    within.isWindow || within.isDocument ? '' : within.element.css('overflow-y'),
                hasOverflowX =
                    overflowX === 'scroll' ||
    ...
    $.ui.plugin.add('draggable', 'scroll', {
        ...
        drag: function (event) {
            ...
            if (i.scrollParent[0] !== document && i.scrollParent[0].tagName !== 'HTML') {
                if (!o.axis || o.axis !== 'x') {
                    if (
                        i.overflowOffset.top + i.scrollParent[0].offsetHeight - event.pageY <
                        o.scrollSensitivity
                    ) {
                ...
                if (!o.axis || o.axis !== 'y') {
                    if (
                        i.overflowOffset.left + i.scrollParent[0].offsetWidth - event.pageX <
                        o.scrollSensitivity
                    ) {
...
;(function ($, undefined) {
            ...
            for (i = 0; i < this._proportionallyResizeElements.length; i++) {
                ...
                if (!this.borderDif) {
                    ...
                    for (j = 0; j < borders.length; j++) {
                        this.borderDif[j] =
                            (parseInt(borders[j], 10) || 0) + (parseInt(paddings[j], 10) || 0)
                    }
                }
    ...
    $.ui.plugin.add('resizable', 'containment', {
        ...
        resize: function (event) {
            ...
            woset = Math.abs(
                (that._helper ? that.offset.left - cop.left : that.offset.left - cop.left) +
                    that.sizeDiff.width,
            )
            hoset = Math.abs(
    ...
    $.widget('ui.sortable', $.ui.mouse, {
        ...
        _intersectsWithPointer: function (item) {
            var isOverElementHeight =
                    this.options.axis === 'x' ||
                    isOverAxis(this.positionAbs.top + this.offset.click.top, item.top, item.height),
                isOverElementWidth =
                    this.options.axis === 'y' ||
                    isOverAxis(
...
;(function ($, undefined) {
    $.widget('ui.menu', {
        ...
        expand: function (event) {
            var newItem =
                this.active && this.active.children('.ui-menu ').children('.ui-menu-item').first()
        ...
        nextPage: function (event) {
            ...
            if (this._hasScroll()) {
                ...
                this.focus(
                    event,
                    this.activeMenu.children('.ui-menu-item')[!this.active ? 'first' : 'last'](),
                )
            }
        },
    ...
    $.widget('ui.progressbar', {
        ...
        _refreshValue: function () {
            ...
            if (this.indeterminate) {
                ...
                if (!this.overlayDiv) {
                    this.overlayDiv = $("<div class='ui-progressbar-overlay'></div>").appendTo(
                        this.valueDiv,
                    )
                }
            } else {

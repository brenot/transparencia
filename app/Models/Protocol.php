...
class Protocol
{
    ...
    {
        return $this->toProtocol($this->findProtocol($id));
    }
    ...
    private function findGeneralProtocol($number)
    {
        ...
        $protocol = $connection->select(
            DB::raw("select * from sy_vw_autor_processo where ano = {$year} and numero = {$number}")
        );

        return $protocol ? json_decode(json_encode($protocol), true)[0] : false;
    }
    ...
    {
        $data = new DataRequest(static::class, config('app.webservice.urls.protocol'), 'GET', [
            $id,
        ]);
    ...
    {
        $result = [$number, ($digits = preg_replace('/\D/', '', $number))];
    ...
    {
        return [$year, $year > 1900 ? $this->toYear($year, 'y') : $this->toYear($year, 'Y')];
    }
